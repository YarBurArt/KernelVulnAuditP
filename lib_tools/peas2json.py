#!/usr/bin/env python3
# source here https://github.com/peass-ng/PEASS-ng/blob/master/parsers/README.md
import json
import re
import sys
        ...
        for reg in regexs:
            ...
            if split_color and len(split_color) > 1:
                ...
                for potential_color_str in split_color:
                    ...
                    color_str = (
                        color_str1 if len(color_str1) < len(color_str2) else color_str2
                    )

                    if color_str:
                        color_str = clean_colors(color_str.strip())
                        # Avoid having the same color for the same string
                        if color_str and not any(
                            color_str in values for values in colors.values()
                        ):
...
def clean_colors(line: str) -> str:
    ...
    line = (
        line.replace("\x1b", "").replace("[0m", "").replace("[3m", "")
    )  # Sometimes that byte stays
    line = line.strip()
    return line
...
def parse_peass(outputpath_loc: str, jsonpath_loc: str = "") -> dict | None:
    global \
        OUTPUT_PATH, \
        JSON_PATH, \
        FINAL_JSON, \
    ...
    OUTPUT_PATH = outputpath_loc
    JSON_PATH = jsonpath_loc
...
if __name__ == "__main__":
    try:
        ...
        print(
            "Error: Please pass the peas.out file and the path to save the json\npeas2json.py <output_file> <json_file.json>"
        )
        sys.exit(1)
