"""Unit tests for presentation.terminal - stdlib-only terminal helpers."""
...
from presentation import glyphs, terminal
from presentation.colors import BOLD, RED, RESET
...
class TestSupportsColor(unittest.TestCase):
    ...
    def test_stream_without_isatty_returns_false(self):
        self.assertFalse(supports_color(object()))

    def test_no_color_env_disables(self):
        os_patch = {**os.environ, "NO_COLOR": "1"}
        with mock.patch.dict(os.environ, os_patch):
            self.assertFalse(supports_color(FakeStream(True)))

    def test_dumb_term_disables(self):
        os_patch = {**os.environ, "TERM": "dumb"}
        with mock.patch.dict(os.environ, os_patch):
            self.assertFalse(supports_color(FakeStream(True)))

    def test_xterm_with_tty_enables(self):
        os_patch = {**os.environ, "TERM": "xterm-256color"}
        with mock.patch.dict(os.environ, os_patch):
            self.assertTrue(supports_color(FakeStream(True)))

    def test_default_stream_is_stdout(self):
        with mock.patch.object(sys, "stdout", FakeStream(False)):
            self.assertFalse(supports_color())


class TestPaint(unittest.TestCase):
    def test_no_color_returns_text_unchanged(self):
        with mock.patch.object(terminal, "supports_color", return_value=False):
            self.assertEqual(paint("hello", RED), "hello")

    def test_empty_color_returns_text_unchanged(self):
        self.assertEqual(paint("hello"), "hello")

    def test_color_wrapped_with_bold(self):
        with mock.patch.object(terminal, "supports_color", return_value=True):
            self.assertEqual(
                paint("hi", RED, bold=True),
                f"{BOLD}{RED}hi{RESET}",
            )
...
class TestIsInteractive(unittest.TestCase):
    def test_both_stdin_and_stdout_tty(self):
        with (
            mock.patch.object(sys, "stdin", FakeStream(True)),
            mock.patch.object(sys, "stdout", FakeStream(True)),
        ):
            self.assertTrue(is_interactive())

    def test_stdin_not_tty(self):
        with (
            mock.patch.object(sys, "stdin", FakeStream(False)),
            mock.patch.object(sys, "stdout", FakeStream(True)),
        ):
...
class TestPager(unittest.TestCase):
    def test_non_interactive_prints_entire_text(self):
        out = io.StringIO()
        with (
            mock.patch.object(terminal, "is_interactive", return_value=False),
    ...
    def test_shorter_than_page_size_prints_all(self):
        out = io.StringIO()
        with (
            mock.patch.object(terminal, "is_interactive", return_value=True),
            mock.patch.object(sys, "stdout", out),
            mock.patch.object(sys, "stdin", FakeStream(True)),
            mock.patch("builtins.input", lambda prompt="": "q"),
        ):
            pager("one\ntwo")
...
class TestSupportsUnicode(unittest.TestCase):
    def test_dumb_term_returns_false(self):
        os_patch = {**os.environ, "TERM": "dumb"}
        with mock.patch.dict(os.environ, os_patch):
            self.assertFalse(supports_unicode(FakeStream(True)))

    def test_linux_term_returns_false(self):
        os_patch = {**os.environ, "TERM": "linux"}
        with mock.patch.dict(os.environ, os_patch):
            self.assertFalse(supports_unicode(FakeStream(True)))

    def test_vt_term_returns_false(self):
        os_patch = {**os.environ, "TERM": "vt100"}
        with mock.patch.dict(os.environ, os_patch):
            self.assertFalse(supports_unicode(FakeStream(True)))

    def test_non_tty_stream_returns_true(self):
        with mock.patch.dict(os.environ, {"TERM": "xterm"}):
            self.assertTrue(supports_unicode(FakeStream(False)))

    def test_utf8_tty_returns_true(self):
        os_patch = {**os.environ, "TERM": "xterm-256color"}
        with mock.patch.dict(os.environ, os_patch):
            self.assertTrue(supports_unicode(FakeStream(True, "utf-8")))

    def test_ascii_tty_returns_false(self):
        os_patch = {**os.environ, "TERM": "xterm-256color"}
        with mock.patch.dict(os.environ, os_patch):
            self.assertFalse(supports_unicode(FakeStream(True, "ascii")))

    def test_missing_encoding_falls_back_to_locale(self):
        os_patch = {**os.environ, "TERM": "xterm-256color"}
        with mock.patch.dict(os.environ, os_patch), mock.patch.object(
            locale, "getencoding", return_value="UTF-8"
        ):
            self.assertTrue(
                supports_unicode(FakeStream(True, None))
            )

    def test_locale_encoding_error_falls_back(self):
        os_patch = {**os.environ, "TERM": "xterm-256color"}
        with mock.patch.dict(os.environ, os_patch), mock.patch.object(
            locale, "getencoding", side_effect=AttributeError
        ):
            self.assertFalse(
                supports_unicode(FakeStream(True, None))
            )
    ...
    def test_stream_encoding_raises_uses_locale(self):
        ...
        with mock.patch.dict(os.environ, os_patch), mock.patch.object(
            ...
            self.assertTrue(
                supports_unicode(RaisingStream(True))
            )


class TestUnicodeGlyph(unittest.TestCase):
    def tearDown(self):
        glyphs._unicode_supported = None

    def test_uses_unicode_when_supported(self):
        with mock.patch.object(glyphs, "supports_unicode", return_value=True):
            self.assertEqual(unicode_glyph("✓", "OK"), "✓")

    def test_uses_ascii_when_not_supported(self):
        with mock.patch.object(glyphs, "supports_unicode", return_value=False):
            self.assertEqual(unicode_glyph("✓", "OK"), "OK")

    def test_caches_support_flag(self):
        glyphs._unicode_supported = None
        with mock.patch.object(glyphs, "supports_unicode") as mocked:
            unicode_glyph("✓", "OK")
            unicode_glyph("✓", "OK")
...
class TestProgressBar(unittest.TestCase):
    ...
    def test_disabled_on_non_tty(self):
        bar = ProgressBar(total=10, stream=self.stream)
    ...
    def test_enabled_property_reflects_tty(self):
        tty = FakeStream(True)
        bar = ProgressBar(total=10, stream=tty)
    ...
    def test_isatty_raises_disables_bar(self):
        ...
        bar = ProgressBar(total=10, stream=RaisingStream())
        self.assertFalse(bar.enabled)

    def test_draw_writes_progress_line(self):
        tty = FakeStream(True)
        bar = ProgressBar(total=10, width=10, label="work", stream=tty)
        bar.update(5)
    ...
    def test_draw_with_color_wraps_bar(self):
        tty = FakeStream(True)
        with mock.patch.object(terminal, "supports_color", return_value=True):
            bar = ProgressBar(total=10, width=10, color=RED, stream=tty)
        self.assertEqual(bar._color, RED)
        bar.update(5)
        self.assertIn(RED, tty.written)
        self.assertIn(RESET, tty.written)

    def test_step_increments_progress(self):
        tty = FakeStream(True)
        bar = ProgressBar(total=4, width=4, stream=tty)
        bar.step()
        bar.step(2)
    ...
    def test_update_with_label_and_note(self):
        tty = FakeStream(True)
        bar = ProgressBar(total=10, width=10, stream=tty)
    ...
    def test_finish_without_total_shows_count(self):
        tty = FakeStream(True)
        bar = ProgressBar(width=10, stream=tty)
        bar.step(3)
        bar.finish()
    ...
    def test_total_negative_clamped_to_zero(self):
        bar = ProgressBar(total=-5, stream=self.stream)
        self.assertEqual(bar._total, 0)

    def test_width_minimum(self):
        bar = ProgressBar(total=1, width=1, stream=self.stream)
